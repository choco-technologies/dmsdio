# Adding a New MCU Port to dmsdio

`dmsdio_port` currently supports `stm32f7`. The port is split from
the core `dmsdio` module so a new architecture can be added without
...
2. Create `src/port/<family>/port.c` implementing the
   `dmod_dmsdio_port_api_declaration(...)` functions declared in
   `include/dmsdio_port.h`, plus `dmod_init`/`dmod_deinit` and any
